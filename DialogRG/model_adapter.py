...
class DualTransformer(nn.Module):
    def __init__(self, config, word_emb, con_emb=None, word_vocab=None, concept_vocab=None, relation_vocab=None, word_rel_vocab=None):
        ...
        self.fuse_linear = nn.Linear(config.d_enc_sent + config.d_enc_concept, config.d_dec) # 修改: fuse the sen & graph encoder
        
        self.linear1 = nn.Linear(config.d_dec, 1024) # 修改
        self.dropout = nn.Dropout(config.dropout) # 修改
        self.linear2 = nn.Linear(1024, config.d_dec) # 修改
        self.activation = _get_activation_fn("relu") # 修改
        self.norm = nn.LayerNorm(config.d_dec)

    

        self.projector = nn.Linear(config.d_dec, 7) # 修改: project represetation dim to num emotion dialog class
        
        self.sent_cross_attn = MultiHeadedAttention(
            config.n_head, config.d_enc_sent, query_dim=config.d_dec, dropout=config.dropout, use_structure=False
        )  # 修改
        n_graph_head = 4 if config.d_enc_concept != 512 else 8 # 修改
        self.graph_cross_attn = MultiHeadedAttention(
                n_graph_head, config.d_enc_concept , query_dim=config.d_dec, dropout=config.dropout, use_structure=False
            ) # 修改
    ...
    def forward(self, batch):
        ...
        if batch["tgt_ref"] is not None:
            ...
            tgt = self.tgt_map(target_ref, self.d_model) # 修改
            tgt = tgt.type(torch.FloatTensor) # 修改
            sent_mem = sent_mem.type(torch.FloatTensor) # 修改
            graph_mem = graph_mem.type(torch.FloatTensor) # 修改
            
            # print("tgt", tgt.size()) # [20,1,512]
            # print("sent_mem", sent_mem.size()) # [20, max_seq, 512]
            # print("graph_mem", graph_mem.size()) # [20, max_seq, 512]
            
            # sent_max_seq = sent_mem.size()[1] # 修改
            # graph_max_seq = graph_mem.size()[1] # 修改
            # max_seq = max(sent_max_seq, graph_max_seq) # 修改
            
            # print("sent_max_seq", sent_max_seq, "graph_max_seq", graph_max_seq, "max_seq", max_seq)
            # print(sent_mem.transpose(1,2).size()) # [20, 512, sent_max_seq]
            # print(graph_mem.transpose(1,2).size()) # [20, 512, graph_max_seq]
            
            # sent_tgt2 = self.sent_cross_attn(
            #     tgt, sent_mem, sent_mem, mask=None, key_padding_mask=sent_mask_inv)[0] # 修改
            # graph_tgt2 = self.graph_cross_attn(
            #     tgt, graph_mem, graph_mem, mask=None, key_padding_mask=graph_mask_inv)[0] # 修改
            
            # 修改
            # self.sent_linear = nn.Linear(sent_max_seq, max_seq)
            # self.graph_linear = nn.Linear(graph_max_seq, max_seq)
            # self.sent_tgt_linear = nn.Linear(max_seq, 1)
            # self.graph_tgt_linear = nn.Linear(max_seq, 1)
            # sent_tgt2 = self.sent_tgt_linear(self.activation(self.sent_linear(sent_mem.transpose(1,2)))).transpose(1,2)
            # graph_tgt2 = self.graph_tgt_linear(self.activation(self.graph_linear(graph_mem.transpose(1,2)))).transpose(1,2)

            # sent_tgt2 = self.sent_tgt_linear(self.activation(self.sent_linear(sent_mem.transpose(1,2)))).transpose(1,2)
            # graph_tgt2 = self.graph_tgt_linear(self.activation(self.graph_linear(graph_mem.transpose(1,2)))).transpose(1,2)

            sent_tgt2 = torch.mean(sent_mem, dim=1)
            graph_tgt2 = torch.mean(graph_mem, dim=1)
            
            # print('sent_tgt2', sent_tgt2.size()) # torch.Size([20, 1, 512])
            # print('graph_tgt2', graph_tgt2.size()) # torch.Size([20, 1, 512])
            tgt2 = sent_tgt2.to(device)
            #tgt2 = self.fuse_linear(torch.cat([sent_tgt2, graph_tgt2], dim=-1).to(device)) # 修改
            # print("tgt2", tgt2.size()) # [20, 1, 512]
            
            tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt2)))) # 修改
            target_emb = self.norm(tgt2) # 修改
            ...
            preds = logits.argmax(dim=1)  # [batch, trg_seq]
            # print("preds", preds.size())
            criterion = torch.nn.CrossEntropyLoss()
            loss = criterion(logits, target_ref.contiguous().view(-1))
            # loss = F.cross_entropy(
            #     logits, # 修改：self.word_vocab_size ——> 7
            #     target_ref.contiguous().view(-1),
            #     ignore_index=0,
            # )
            # print(logits)
            # print(preds)
            # print(target_ref.contiguous().view(-1))
            
            acc = accuracy_score(batch["tgt_ref"].cpu(), preds.cpu()) # 修改
            precision = metrics.precision_score(batch["tgt_ref"].cpu(), preds.cpu(), labels=[0,1,2,3,4,5,6], average='macro', zero_division=0)
            recall = metrics.recall_score(batch["tgt_ref"].cpu(), preds.cpu(), labels=[0,1,2,3,4,5,6], average='macro', zero_division=0)
            f1_score = metrics.f1_score(batch["tgt_ref"].cpu(), preds.cpu(), labels=[0,1,2,3,4,5,6], average='macro', zero_division=0)
    ...
    def tgt_map(self, tgt, d_model): # 修改 tgt: [batch, trg_seq]
        # print(tgt.size()) #torch.Size([20,1])
        _, tgt_len =tgt.size()
        tgt = tgt.repeat(1, d_model).view(-1, tgt_len, d_model)
        return tgt
...
logits = torch.rand(4, 7)
target_ref = torch.tensor([[0], [0], [1], [6]])
# print("logits", logits.size())
preds = logits.argmax(dim=1)  # [batch, trg_seq]
print(preds)
print(target_ref.contiguous().view(-1))
...
##acc = accuracy_score(torch.tensor([[0], [0]]).cuda(), preds.cuda())
#print(acc)