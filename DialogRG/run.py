...
def train(model, data_loader, optimizer, n_gpu, FLAGS):
    ...
    for i, _data in tqdm.tqdm(enumerate(data_loader)):
        sv, sv_len, cv, cv_len, rv, word_rel_mask, wr, t = _data
        batch = {}
        
        t_len = len(t)
        batch["tgt_ref"] = torch.as_tensor(t).reshape(t_len, 1) #修改
        ...
        # print('train batch', i, ':', outputs["accuracy"])
        # print(batch["src_len"]) # tensor([ 54, 158,  40,  44, 198,  60,  90,  37,   9,  12,  81, 227,  11, 145, 72, 146,  56,  21,  98,  16])
        # print(batch["con_len"]) # tensor([ 37, 112,  30,  34, 117,  42,  65,  23,   8,  12,  59, 159,   8,  96, 51,  96,  41,  20,  64,  13])
        train_right += outputs["accuracy"] # 修改
        train_precision += outputs["precision"]
        train_recall += outputs['recall']
        train_f1 += outputs['f1_score']
        train_total += 1 # 修改
        torch.cuda.empty_cache()
    
    return train_loss / train_step, train_right / train_total, train_precision / train_total, train_recall / train_total, train_f1 / train_total
...
def validate(model, data_loader, n_gpu):
    ...
    with torch.no_grad():
        ...
        dev_precision, dev_recall, dev_f1 = 0.0, 0.0, 0.0
        for i, _data in tqdm.tqdm(enumerate(data_loader)):
            ...
            # print('dev batch', i, ':', outputs["accuracy"])
            dev_right += outputs["accuracy"] # 修改

            dev_precision += outputs["precision"]
            dev_recall += outputs['recall']
            dev_f1 += outputs['f1_score']

            dev_total += 1 # 修改
            torch.cuda.empty_cache()
    
    return dev_loss / dev_step, dev_right / dev_total, dev_precision / dev_total, dev_recall / dev_total, dev_f1 / dev_total
...
if __name__ == "__main__":
    ...
    for iter in range(start_epoch, FLAGS.num_epochs):
        train_loss, train_accu, train_prec, train_recall, train_f1 = train(model, train_loader, optimizer, n_gpu, FLAGS)
        val_loss, val_accu, dev_prec, dev_recall, dev_f1= validate(model, dev_loader, n_gpu)

        print(
            "iter: {}, lr:{:.5f} TRAIN loss: {:.4f} accu: {:.4f} prec: {:.4f} recall {:.4f} f1 {:.4f}; VAL loss: {:.4f} accu: {:.4f} prec: {:.4f} recall {:.4f} f1 {:.4f}".format(
                iter, optimizer.param_groups[0]["lr"],
                train_loss, train_accu, train_prec, train_recall, train_f1,
                val_loss, val_accu, dev_prec, dev_recall, dev_f1
            )
        )
        log_file.write(
            "iter: {}, lr:{:.5f} TRAIN loss: {:.4f} accu: {:.4f} prec: {:.4f} recall {:.4f} f1 {:.4f}; VAL loss: {:.4f} accu: {:.4f} prec: {:.4f} recall {:.4f} f1 {:.4f}".format(
